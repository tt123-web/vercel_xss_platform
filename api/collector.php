...
$headers_data = function_exists('getallheaders') ? getallheaders() : array();
...
if ($decoded_get_data) $info['decoded_get_data'] = stripArr($decoded_get_data);
$info['post_data'] = stripArr($post_data);
if ($decoded_post_data) $info['decoded_post_data'] = stripArr($decoded_post_data);
$info['cookie_data'] = stripArr($cookie_data);
if ($decoded_cookie_data) $info['decoded_cookie_data'] = stripArr($decoded_cookie_data);
$info['keepsession'] = isKeepSession($info) ? true : false;

save_xss_record(json_encode($info), $request_time);
if (defined('MAIL_ENABLE') && MAIL_ENABLE) {
